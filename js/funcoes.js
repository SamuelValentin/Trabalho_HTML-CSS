...
function cadastrarProduto(idCat, produto, codig, descricao, preco, peso, img, qtidade) {
    /* Cadastrando no backend do professor */
     ...
     let url = 'http://loja.buiar.com/?key=cZbWpEr0MD&f=json&c=produto&t=inserir&nome="'+produto+'"&codigo='+codig+'&categoria=937'+'&descricao="'+descricao+'"&peso='+ peso +'&preco='+preco+'&imagem="'+img+'"';
...
function validarPedido(nomeCliente, CPF, CEP, rua, numRua, comp, bairro, cidade, uf) {
    let nomeP = document.getElementById(nomeCliente).value;
    let cpfP = document.getElementById(CPF).value;
    let cepP = document.getElementById(CEP).value;
    let ruaP = document.getElementById(rua).value;
    let numP = document.getElementById(numRua).value;
    let compP = document.getElementById(comp).value;
    let bairroP = document.getElementById(bairro).value;
    let cidadeP = document.getElementById(cidade).value;
    let ufP = document.getElementById(uf).value;
    ...
    if (nomeP == "")
        ...
        alert("Numero da rua não pode estar em branco. Favor preenchê-lo!");
    else cadastrarPedido(nomeP, cpfP, cepP, ruaP, numP, compP, bairroP, cidadeP, ufP);
}

function cadastrarPedido(nome, cpf, cep, rua, num, comp, bairro, cidade, uf) {
    /* Cadastrando no backend do professor */
    var xhr = new XMLHttpRequest();

    let url = 'http://loja.buiar.com/?key=cZbWpEr0MD&f=json&c=pedido&t=inserir&nome='+nome+"&cpf=" + cpf + '&cep=' + cep + '&rua=' + rua + '&numero=' + num + '&bairro=' + bairro + '&cidade=' + cidade + '&uf=' + uf + '"';
    ...
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
    xhr.send();
    //let idPedido = {id} Precisa pegar o Id do backend do professor
...
function incluiProdPed() {
    id = 294;
    //Receber lista de produtos do carrinho
    codigo = ;
    categoria = ;
    nome = ;
    descricao = ;
    ...
    let url = 'http://loja.buiar.com/?key=cZbWpEr0MD&f=json&c=pedido&t=listar&id=' + id + ;
