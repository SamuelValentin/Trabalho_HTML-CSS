...
//-------------Pegar info do carrinho para pedidos--------------
function criarCarrinhoModal() {
    if (typeof (Storage) !== "undefined") {

        let produtosCarrinho = localStorage.getItem("produtos");//produtos seria o nome do array
        if (produtosCarrinho == null) {
            //Alerta -> Seu carrinho está vazio!
            console.log('VAZIO')
        } else {
            produtosCarrinho = JSON.parse(produtosCarrinho);
            produtosCarrinho.forEach(produtosCarrinho => {//é como um for q pega todos os items
                produto.id;
                produto.preco;//Para cada item que tem em "produtos"
                //produtosCarrinho.nome ... produtosCarrinho.preco...
            }
          }
    }
}
